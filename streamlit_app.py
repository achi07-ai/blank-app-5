...
st.markdown(f"""
    ...
    .fc-col-header-cell.fc-day-sat {{ color: #007bff; }}
    .fc-col-header-cell.fc-day-sun {{ color: #dc3545; }}
        ...
        with st.form("edit_form"):
            ...
            if st.form_submit_button("更新", use_container_width=True):
                ...
                supabase.table("todos").update({
                    "title": new_title, "category": new_cat, "start_at": ns, "end_at": ne,
                    "reminder_at": calculate_reminder(new_date, new_cat), "is_complete": is_done
...
with st.sidebar:
    ...
    with st.expander("🔐 パスワード変更"):
        npw = st.text_input("新パスワード", type="password", key="pw_in")
        cpw = st.text_input("確認用", type="password", key="pw_conf")
        if st.button("更新", key="pw_btn") and npw == cpw:
...
# --- 9. メイン画面 ---
st.markdown(f"<h1 class='main-title'>{APP_NAME}</h1>", unsafe_allow_html=True)
var_sal, fix_sal, hours = get_salary_info(current_todos, settings['hourly_wage'], settings['fixed_salary'])
total_sal = var_sal + fix_sal

col_sal, col_rem = st.columns([1, 2])
with col_sal:
    st.markdown(f"""<div class="salary-box"><p style='margin:0; font-size:0.9em; color:#666;'>💰 今月の見込み合計</p><h2 style='margin:0; color:#9B59B6;'>¥{total_sal:,}</h2><div class="salary-detail">内訳: 固定 ¥{fix_sal:,} + 時給分 ¥{var_sal:,}<br>(バイト合計: {hours} 時間)</div></div>""", unsafe_allow_html=True)
...
cal_options = {
    ...
    "initialView": "dayGridMonth", "locale": "ja", "slotMinTime": "00:00:00", "slotMaxTime": "24:00:00", "editable": True, "eventDisplay": "block",
}
state = calendar(events=events, options=cal_options, key="manetime_final_stable")
